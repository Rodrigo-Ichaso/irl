...
function formatVerdict(verdict) {
    ...
    const reasons = risk?.reasons?.join(", ") || "none";
    const lines = [
        `VERDICT: ${decision}`,
...
const server = new Server({ name: "irl-mcp", version: "0.1.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "evaluate_intent",
            description: "REQUIRED before any risky action: file deletion, production writes, " +
